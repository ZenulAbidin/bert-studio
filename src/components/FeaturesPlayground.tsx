...
export const FeaturesPlayground: React.FC<{ loadedModels: string[] }> = ({ loadedModels }) => {
  ...
  const handleExtract = async () => {
    ...
    setFeatures(null);
    try {
      const response = await apiClient.post('/features', {
        text: inputText,
        model: selectedModel,
      });
      ...
      toast.error(error?.response?.data?.detail || 'Feature extraction failed.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold">Feature Extraction Playground</h2>
      <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Enter text to extract features..."
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
          />
        </div>
        {/* Action Button */}
