...
export const NERPlayground: React.FC<{ loadedModels: string[] }> = ({ loadedModels }) => {
  ...
  const handleNER = async () => {
    ...
    setEntities(null);
    try {
      const response = await apiClient.post('/ner', {
        text: inputText,
        model: selectedModel,
      });
      ...
      toast.error(error?.response?.data?.detail || 'NER failed.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold">NER Playground</h2>
      <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Enter text for NER..."
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
          />
        </div>
        {/* Action Button */}
